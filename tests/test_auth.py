...
def test_missing_header_is_401(client):
    resp = client.get("/v1/keys/orders:1")
    assert resp.status_code == 401


def test_malformed_header_is_401(client):
    resp = client.get("/v1/keys/orders:1", headers={"Authorization": "tok-orders"})
    assert resp.status_code == 401


def test_unknown_token_is_401(client):
    resp = client.get("/v1/keys/orders:1", headers=_auth("not-a-real-token"))
    assert resp.status_code == 401


def test_valid_token_wrong_namespace_is_403(client):
    resp = client.get("/v1/keys/inventory:1", headers=_auth("tok-orders"))
    assert resp.status_code == 403


def test_valid_token_wrong_verb_is_403(client):
    # tok-readonly only has "read" on inventory, not "write"
    resp = client.put("/v1/keys/inventory:1", json={"value": "x"}, headers=_auth("tok-readonly"))
    assert resp.status_code == 403


def test_valid_token_correct_namespace_and_verb_succeeds(client):
    put_resp = client.put("/v1/keys/orders:1", json={"value": "bar"}, headers=_auth("tok-orders"))
    assert put_resp.status_code == 200

    get_resp = client.get("/v1/keys/orders:1", headers=_auth("tok-orders"))
...
def test_default_namespace_used_when_no_colon_in_key(client):
    ...
    resp = client.get("/v1/keys/plainkey", headers=_auth("tok-orders"))
    assert resp.status_code == 404  # allowed through auth, just doesn't exist

    resp = client.put("/v1/keys/plainkey", json={"value": "x"}, headers=_auth("tok-orders"))
...
def test_auth_disabled_when_no_tokens_configured():
    ...
    resp = client.put("/v1/keys/foo", json={"value": "bar"})
    assert resp.status_code == 200
